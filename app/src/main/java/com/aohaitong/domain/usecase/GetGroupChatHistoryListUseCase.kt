...
) : SingleUseCase<Long, List<ChatMsgBusinessBean>>() {
    override fun execute(parameter: Long): Single<List<ChatMsgBusinessBean>> {
        return Single.just(dataBaseRepository.getGroupChatHistoryList(parameter).map {
