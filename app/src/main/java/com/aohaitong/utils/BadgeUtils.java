...
public class BadgeUtils {
    ...
    public static boolean setCount(final int count, final Context context, Notification notification) {
        if (count >= 0 && context != null) {
            Log.d(CommonConstant.LOGCAT_TAG, "手机厂商：" + Build.BRAND);
            switch (Build.BRAND.toLowerCase()) {
                case "xiaomi":
                case "redmi":
                    setNotificationBadge(count, notification);
                    return true;
                case "huawei":
                case "honor":
                ...
                default:
                    return setNotificationBadge(count, notification);
            }
        } else {
            return false;
        }
    ...
    public static boolean setNotificationBadge(int count, Notification notification) {
        try {
            ...
            Log.e("wwwww", "发送通知状态: " + e.getMessage());
            e.printStackTrace();
        }
        return true;
    }
